...
计费规则：
1.tokenplan/codingplan-》按照时间时间为范围，限制小时/天/周 消耗额度 —— 包年/包月
2.api计费->按照token消耗直接计算，提前充值

确定项目需求和核心功能，明确用户输入、系统处理过程以及最终输出结果 -》 生成 CLAUDE.md(项目全局记忆 -》 给大模型看)
